...
class MappingRequestResolver(private val definition: InternalMappieDefinition) : BaseVisitor<List<MappingRequest>, MappieContext>() {
    ...
    override fun visitFunction(declaration: IrFunction, data: MappieContext): List<MappingRequest> {
        return if (declaration.accept(ShouldTransformCollector(data), Unit)) {
            context(data) { MappingResolverSelector.select(definition).resolve(definition, declaration) }
        } else {
            emptyList()
        }
    }
